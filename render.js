...
const DEFAULT_TITLE = 'CineBox · Актуални Филми и Сериали: Сюжет, Оценка, Актьорски Състав и Трейлър';
const DEFAULT_DESC = 'В CineBox ще намерите сюжет, актьорски състав, оценка и официален трейлър на най-популярните филми и сериали в момента, с пълна информация за сезони и епизоди.';
const DEFAULT_OG_IMAGE = 'https://placehold.co/1200x630/17171b/8d8a92?text=CineBox';

// TODO: въведете кода за верификация на Google Search Console ЗА ТОЗИ домейн
// (кодът на cinebox-kr НЕ може да се използва тук -- той е свързан с друг домейн).
...
// ---------- Adsterra & Histats ----------
// ЗАБЕЛЕЖКА: ключовете/зоните по-долу са СЪЩИТЕ като cinebox-kr/th/de/br/fr/vn/es -> статистиката
// ще се смесва. Ако искате да ги разделите, създайте нови зони в Adsterra и Histats за този домейн.
...
// Popunder не се активира по подразбиране (риск от "натрапчив интерстициал" и лошо UX).
// Разкоментирайте 3-те реда по-долу + извикайте popunderScript() в layout() преди </body>, ако все пак искате.
...
<header>
  <div class="header-inner">
    ...
    <nav class="tabs">
      <a class="tab-btn ${activeTab === 'movie' ? 'active' : ''}" href="/movie">Филми</a>
      <a class="tab-btn ${activeTab === 'tv' ? 'active' : ''}" href="/tv">Сериали</a>
    </nav>
    <div class="search-wrap">
      <input id="search-input" type="text" placeholder="Търсене на заглавие..." autocomplete="off">
...
function posterCard(item, type) {
  ...
  return `
    <a class="poster-card" href="/${type}/${item.id}/${encodeURIComponent(slug)}">
      ...
      <div class="poster-sub">${date || 'Неизвестна година'}</div>
    </a>
  `;
}
...
function trailerBlock(videos) {
  ...
  if (!trailer) return `<div class="no-trailer">Няма наличен трейлър.</div>`;
  return `
...
function castGrid(credits) {
  ...
  if (!cast.length) return `<div class="empty">Няма налична информация за актьорския състав.</div>`;
